...
plt.plot(mags,Fre+Hmax*LY/2,label="Transfer Matrix free energy $F(\mu)+\mu L/2$")
...
for nb,Hm in enumerate(mags):
    ...
    tm /= (3*hn)/(Hmax-Hm)
    Int[int(nb/2)] = tm
plt.plot(mags[2::2],Int[1:],'+',label="Transfer Matrix integration, $\mu_2=1$")
#plt.plot(mags,Mag,'+')
...
for nb,Hm in enumerate(mags):
    ...
    tm /= (3*hn)/(Hmax-Hm)
    Int[int(nb/2)] = tm
plt.plot(mags[2::2],Int[1:],label="Monte Carlo integration, $\mu_2=1$")
...
plt.ylabel('$F(\mu_1)-F(\mu_2)$')
plt.xlabel('$\mu_2$')
plt.savefig('integration-free-ene-negstagged.pdf')
plt.show()
plt.close()
