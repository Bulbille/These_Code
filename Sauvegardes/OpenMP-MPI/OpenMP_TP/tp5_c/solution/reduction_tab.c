...
#pragma omp parallel
  ...
  double tab1[nmol], tab2[nmol], tab2loc[nmol];
  clear(tab2, nmol);

  clock_t t_cpu_0;
  struct timeval t_elapsed_0;

#pragma omp parallel private(tab1, tab2loc)
  {
...
#pragma omp for schedule(static)
    for(int k=0; k<nmolec; k++) {
      ...
      for(int l=0; l<nmol; l++) tab2loc[l] += 2*tab1[l];
    }

    // Mise à jour de tab2 à partir de tab2loc
#pragma omp critical
