...
plt.plot(mags,Fre,label="Transfer Matrix free energy ")
...
for nb,Hm in enumerate(mags):
    ...
    tm /= (3*hn)/(Hmax-Hm)
    Int[int(nb/2)] = tm
plt.plot(mags[2::2],-Int[1:],label="Monte Carlo integration, $B_2=2$")
...
for nb,Hm in enumerate(mags):
    ...
    tm /= (3*hn)/(Hmax-Hm)
    Int[int(nb/2)] = tm
plt.plot(mags[2::2],-Int[1:],label="Monte Carlo integration, $B_2=6$")
