...
int       LX    = 70, LY = 30;
int       Hn = 2e1; double Hmax = 3;
long int  T_EQ = 1e4,  T_MAX = 1e6;
...
int main(int argc,char* argv[]){
    ...
    std::uniform_int_distribution<int> rand_01_int(0,1);
    ...
    for(int k = 0; k<Nb_H ; k++){
        ...
        for(int t = 0; t<T_EQ ; t++){
            ...
            ajout   = 2*rand_01_int(generator)-1;
    ...
    exit();
        ...
        cout << (clock()-cputime)/CLOCKS_PER_SEC << "\n";
        delete[] system;
    ...
    if(rang == 0){
        ...
        for(int i=0;i<Hn;i++)
            fmag << 1.*i*Hmax/(Hn-1) << " " << valtot[i] <<  " " << sqrt((ecartTot[i]-pow(valtot[i],2))*2*1e3*LX/T_MAX) << "\n";
    }
    MPI_Finalize();
return 0;
}
