...
// 2<<0 = 2, 2<<n = 2^(n+1)
int  Hn = 4;
int  Ndiv = 4, pas = Hn/Ndiv;
double T_EQ = 3e6, T_MAX = 5e6;
double Hmax = 1e-1;
...
int main(int argc,char* argv[]){
    ...
    for(int k = rang*Nb_H; k<(rang+1)*Nb_H ; k++){
        ...
        ecartTyp[cmpt] = 0;
        skew[cmpt] = 0;
        int* distrib = new int[LY];
        for(int y=0;y<LY;y++) distrib[y]=0;
        ...
        str = prefix+"/X"+to_string(LX)+"_H"+streamH.str()+"_F"+to_string(k)+"-distribution";
        ofstream fmag(str.c_str(),std::ofstream::out);
        for(int y=0;y<LY;y++)
            fmag<< y << " " << distrib[LY] << endl;
        fmag.close();
        delete[] system;
