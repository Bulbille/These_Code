...
for k in np.arange(np.size(data[:,0])) :
    if k%2 != 0 or k==0:
        continue
    mags[k/2] = data[k,0]
    FreSim[k/2] = FreSim[k/2-1] - (mags[k/2]-mags[k/2-1])/6 * ( data[k-2,1] + 4*data[k-1,1] + data[k,1] )
...
plt.subplot(211)
plt.plot(mags,FreSim,label='Simulation '+model)
plt.plot(mags,FreTM,label='TM '+model)
### Plot de la magnétisation
plt.subplot(212)
plt.errorbar(data[:,0],data[:,1],yerr=data[:,2],label='Simulation '+model)
plt.plot(data[:,0],magTM,label='TM '+model)
...
#plt.xlim(-0.005,0.3)
plt.legend(fontsize=fsize-2,loc="center right")
