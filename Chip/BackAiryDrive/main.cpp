...
int main(int argc,char* argv[]){
    ...
    for(int k = rang*Nb_f; k<(rang+1)*Nb_f ; k++){
        ...
        cout << k << " " << dmin << " " << dmax <<  " " << Dn << " "  << drive << " " << dmin+(dmax-dmin)/(Dn-1)*k<< endl;
        continue;
    ...
    //****** CALCUL TEMPS DE CORRÉLATION *********//
        ...
        fhisto.close();
        str = prefix+"/snap"+to_string(LX)+"Ttc"+streamT.str()+ "_mu"+streamM.str()+"_f"+to_string(drive);
    ...
    MPI_Gather(valeurs ,Nb_f,MPI_DOUBLE,valTot,Nb_f,MPI_DOUBLE,0,MPI_COMM_WORLD);
    ...
    MPI_Gather(Tcor,Nb_f,MPI_DOUBLE,TcorTot,Nb_f,MPI_DOUBLE,0,MPI_COMM_WORLD);
    ...
    if(rang == 0){
        ...
        for(int i=0;i<Dn;i++)
            fmag << logspace(i,dmin,dmax,Dn) << "\t" << valTot[i] <<  "\t\t" <<  ecartTot[i] << "\t\t" << J*eneTot[i]<< " " << TcorTot[i] << "\n";
        fmag.close();
    }
    MPI_Finalize();
    delete[] valeurs;
