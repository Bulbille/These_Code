...
for nt,temp in enumerate(tempspace) :
    ...
    for nm,mu in enumerate(muspace):
        ...
        chi[nm] = popt[0]
    chiplot.plot(muspace,chi,label='$\\beta = '+str(temp)+'$',color=colors[nt])
    ide     = np.where((muspace>1e-3)& ( muspace < 0.8))
    popt,pcov = curve_fit(powlaw,muspace[ide],chi[ide],p0=[-0.33,1])
