...
int main(int argc,char* argv[]){
    ...
    for(int k = rang*Nb_mu; k<(rang+1)*Nb_mu ; k++){
        ...
        energies[cmpt] = 0;
        ecartTyp[cmpt] = 0;
        double mu =  normspace(k,mumin,mumax,Mun); //Le -1 est pour arriver à mumax lorsque k = Nb_mu au rang supérieur
    //    cout << k << " " << mu << endl;
...
//        Tcor[cmpt] = temps;
    ...
    if(rang == 0){
        ...
        for(int i=0;i<Mun;i++)
            fmag << normspace(i,mumin,mumax,Mun)<< "\t" << valTot[i] <<  "\t\t" <<  ecartTot[i] << "\t\t" << J*eneTot[i]<< " " << TcorTot[i] << "\n";
        fmag.close();
    }
    MPI_Finalize();
    delete[] valeurs;
