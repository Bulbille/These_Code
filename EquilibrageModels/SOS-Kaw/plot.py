...
for nt,temp in enumerate(tempspace):
    print(nt,temp)
    popt,pcov = curve_fit(expeq,big_eq[temp][:200,0],big_eq[temp][:200,1],p0=[1,1,100])
    pequi.plot(big_eq[temp][:,0],big_eq[temp][:,1],color=colors[nt],label="$T="+str(temp)[:4]+'$,$\\tau_{eq} = '+str(int(popt[0]))+'$')
    pequi.plot(big_eq[temp][:,0],expeq(big_eq[temp][:,0],*popt),'-.',color=colors[nt])

    popt,pcov = curve_fit(expcor,big_cor[temp][:500,0],big_cor[temp][:500,1],p0=[100,0.5])
    print(popt)
    pcore.plot(big_cor[temp][:,0],big_cor[temp][:,1],color=colors[nt],label="$T="+str(temp)[:4]+'$,$\\tau_{cor} = '+str(int(popt[0]))+'$')
...
plt.savefig('sos-kaw-eq-cor.pdf')
plt.show()
plt.close()
