...
const double    dx           = 0.1;
const int       LX           = 5000;
double          dt           = 0.1; // dt est calculé à chaque pas de temps par cfl()
// Paramètres sauvegarde dans fichiers
const long int  T_PHOTO      = 1e1;
const long int  T_DONNEE     = T_PHOTO*1e4;
const int       histo_min    = -1e2;
const int       histo_max    = -histo_min;
const int       histo_bin    = histo_max*5e0;
string          prefix       = ".";
string          suffix       = "";
string          algo;
...
//        cfl(&dt,grille);
        ...
        for(int x=0 ; x<LX ; x++)
            grille[x] = grille_tmp[x]- 1./LX*(somme_mag+somme_alea);
    }
