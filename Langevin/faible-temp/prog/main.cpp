...
const double    dx           = 0.1;
const int       LX           = 500;
double          dt           = 0.001; // dt est calculé à chaque pas de temps par cfl()
...
const int       histo_bin    = histo_max*2e0;
string          prefix       = ".";
string          suffix       = "";
string          algo;
    ...
    for(double t = 0 ; true; t+= dt){
        ...
        if(photo_suiv <= static_cast<int>(t) && static_cast<int>(modulo(t,T_PHOTO)) == 0) {
            ...
            if(donnee_suiv <=static_cast<int>(t)){ 
                ecriture(grille,dt,moyenne,*histo);
        ...
        for(int x=0 ; x<LX ; x++){
            ...
            contrainte += gauss + H*sign(grille[x]);

            grille_tmp[x] = grille[x]
               // + dx * sigma * (grille[modulo(x+1,LX)] - 2*grille[x] + grille[modulo(x-1,LX)])
                + dx * sigma * (sign(grille[modulo(x+1,LX)] - grille[x]) - sign(grille[modulo(x-1,LX)]-grille[x]) )
...
void cfl(double* delta_t, double grille[]){
    ...
    for(int i=0 ; i<LX; i++)
        if( abs(grille[modulo(i+1,LX)] - grille[modulo(i-1,LX)]) > max)
            max =(grille[i] != 0)* (abs(grille[modulo(i+1,LX)] - grille[modulo(i-1,LX)]));//abs(grille[i]);
    double dt = 2*sigma + cisaillement*max;
//    if(dt >= 1e7) dt = 1e7;
    *delta_t = dx / dt;
}
...
void ecriture(double* grille,double temps, double moyenne,gsl_histogram histogramme){
    ...
    thermo_res << ttc   << " " << temps << " " << cisaillement << "\n";
    thermo_res.close();
