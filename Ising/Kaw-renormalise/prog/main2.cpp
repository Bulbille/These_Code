...
/******************************************/
	...
	for(double t = 0; t<=T_MAX ; t+= t_step){
		...
		int modulo_photo = static_cast<int>(modulo_d(t_step+t,T_PHOTO));
                if(nb_photos > 0 && modulo_photo != 0 && static_cast<int>((t+t_step)/T_PHOTO) != nb_photos) {;}
		else {
			nb_photos++;
...
for(int x=0;x<TAILLE_X;x++){
for(int y=0;y<TAILLE_Y;y++){
	p_mag += grille_renormalisee[x][y];
	p_e += hamil_loc(grille_renormalisee,x,y)/2;
}
}
...
for(int x=0;x<TAILLE_X;x++){
	...
	for(int y=0;y<TAILLE_Y;y++){
		p_magx[x] += 1. * grille_renormalisee[x][y] / TAILLE_X;
		p_ex[x] -= 1. * grille_renormalisee[x][y] * grille_renormalisee[modulo(x+1, TAILLE_X)][y] / TAILLE_X;
		p_ey[x] -= 1. * grille_renormalisee[x][y] * grille_renormalisee[x][modulo(y+1, TAILLE_Y)] / TAILLE_X;
...
for(int x=0;x<TAILLE_X;x++){
	...
	for(int y=0; y<TAILLE_Y;y++){
		...
		correl_y[x][y] = 1.*correl_y[x][y]*(nb_photos-1)/nb_photos+1.*p_correl_y[x][y]/nb_photos/n_correl[x][y];
	}
}

ecriture(grille_renormalisee,magx, ex, ey,magnetisation, e_int, chi, c_v,static_cast<int>(t), p_correl_y);
		}
